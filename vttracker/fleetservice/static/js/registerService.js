...
function validation(form){
  ...
  */
  ...
  result.push(formValidator.addValidation('from_depto', 'required', 'El campo no debe estar vacio.'));
  result.push(formValidator.addValidation('subject', 'required', 'El campo no debe estar vacio.'));
...
$j('#btnSaveservice').click(function(e){
  ...
  //validation
  let form = document.querySelector('#formSaveService');
  let result = validation(form);

  if(result){

    let formData = jsonFormat($j('#formSaveService').serializeArray());
...
$j(':input[name=end_kilometer], :input[name=subject], :input[name=from_depto]')
  .focus(function(){
    if($j(this).val() != ''){
      $j(this).val('');
    }
